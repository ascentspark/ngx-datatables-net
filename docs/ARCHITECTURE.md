...
constructor() {
  afterNextRender(() => {                    // browser-only, SSR-safe for free
    ...
    this.bindEvents(this.table);             // table.on('draw' / 'select' / 'page' / 'xhr', handler)
...
- **HTML renderers are opt-in + funnelled through a sanitizer hook.** When a consumer wants an HTML
  ...
  `DomSanitizer.sanitize(SecurityContext.HTML, value)` before they reach DataTables. Document loudly
